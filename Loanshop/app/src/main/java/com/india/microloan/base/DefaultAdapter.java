...
public abstract class DefaultAdapter<T> extends RecyclerView.Adapter<BaseHolder<T>> {
    ...
    public BaseHolder<T> onCreateViewHolder(ViewGroup parent, final int viewType) {
        View view = LayoutInflater.from(UIUtils.getContext()).inflate(getLayoutId(viewType), parent, false);
        ...
        mHolder.setOnItemClickListener(new BaseHolder.OnViewClickListener() {//设置Item点击事件
            ...
            public void onViewClick(View view, int position) {
                if (mOnItemClickListener != null && mInfos.size() > 0) {
                    mOnItemClickListener.onItemClick(view, viewType, mInfos, position);
                }
            }
        });
        return mHolder;
    ...
    public interface OnRecyclerViewItemClickListener<T> {
        void onItemClick(View view, int viewType, List<T> data, int position);
    }
